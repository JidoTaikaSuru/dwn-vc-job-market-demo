...
import {createNewEmbeddedWalletForUser, getUserEmbeddedWallet,} from "~/lib/embeddedWalletLib";
...
export default function Login() {
    ...
    const { session, setWallet } = useContext(SessionContext);
    ...
    const logUserIntoAppThenRedirect = async (
    ...
    const emailPassSubmit: SubmitHandler<VerifyEmailPasswordFormProps> = async (
        ...
        try {
            ...
            if (!data.user) {
                ...
                await logUserIntoAppThenRedirect(signupData.session, pin);
                ...
                await logUserIntoAppThenRedirect(data.session, pin);
            }
        } catch (error: any) {
            console.log("error", error);
            setAdditionalError(error.message);
