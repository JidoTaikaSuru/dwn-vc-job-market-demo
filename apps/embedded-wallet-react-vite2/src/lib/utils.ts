...
import { DEBUGGING, myDid, user, web5 } from "@/lib/common.ts";
import { selector, selectorFamily } from "recoil";
import type { Record } from "@web5/api";
import type { ProtocolDefinition } from "@tbd54566975/dwn-sdk-js";
import { DateSort, ProtocolsQueryFilter } from "@tbd54566975/dwn-sdk-js";

export function cn(...inputs: ClassValue[]) {
  return twMerge(clsx(inputs));
}

const logIfDebug = (message: string) => {
...
>({
  ...
  get:
    ({ fromDWN, protocol }) =>
    async () => {
      return await dwnQueryOtherDWNByProtocol(fromDWN, protocol);
    },
});

export const dwnQueryOtherDWNByProtocol = async (
  fromDWN: string,
  protocol: ProtocolDefinition,
) => {
  console.groupCollapsed("dwnQueryOtherDWNByProtocol");
  console.debug(
...
export const dwnQueryOtherDWNArbitraryFilter = selectorFamily<
  Array<DwnListType>,
  {
    fromDWN: string;
...
>({
  ...
  get:
    ({ fromDWN, filter }) =>
    async () => {
      console.groupCollapsed("dwnQueryOtherDWNArbitraryFilter");
...
export const dwnReadOtherDWN = async (
  ...
  console.groupCollapsed("dwnReadOtherDWN");
  console.debug(
...
export const dwnQuerySelf = async (
  ...
  console.debug("dwnQuerySelf ~ protocol:", protocolUrl);
  try {
    const { records } = await web5.dwn.records.query({
      message: {
        filter: {
...
>({
  ...
  get:
    ...
    async () => {
      ...
      return await dwnQueryOtherDWNByProtocol(did, selfProfileProtocol);
    },
});
...
>({
  ...
  get:
    ...
    async () => {
      if (!did) return undefined;
      return await dwnQueryOtherDWNByProtocol(
        did,
...
>({
  ...
  get: async () => {
    return await dwnReadSelfReturnRecordAndData();
  },
});

export const dwnReadSelfReturnRecordAndData = async () => {
  console.log(
    "🚀 ~ file: utils.ts:200 ~ dwnReadSelf ~ protocol:",
