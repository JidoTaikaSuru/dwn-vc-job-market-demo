...
import { dwnCreateJobPost } from "./lib/utils";
...
<Label>
{defaultJobTitle}
        <Input name="titleInput" onChange={(e) => setJobTitle(e.target.value)} />
       
      </Label>
      
<Label>
{defaultJobDescr}
        <Input name="descrInput" onChange={(e) => setJobDescription(e.target.value)} />

        </Label>
        {jobTitle && jobDescription ? (<Button onClick={() => postJob()}>Create Job</Button>) : (<> </>)}
